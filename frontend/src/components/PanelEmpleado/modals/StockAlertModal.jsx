...
const StockAlertModal = ({ open, payload, onClose }) => {
  if (!open) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
        <h3 className="text-xl font-semibold mb-3">⚠️ Alerta: Stock Bajo</h3>
        {items.length === 0 ? (
          <p>No hay medicamentos en la alerta.</p>
        ) : (
            ...
            {items.map((m) => (
              <li key={m.id} className="flex justify-between items-center border-b py-2">
                <div>
                  ...
                  <div className="text-sm text-gray-500">Disponible: {m.stock_actual} / Mínimo: {m.stock_minimo}</div>
                </div>
                <div className="text-sm text-red-600 font-semibold">{m.stock_actual <= m.stock_minimo ? 'Bajo' : ''}</div>
              </li>
            ))}
          </ul>
        )}
